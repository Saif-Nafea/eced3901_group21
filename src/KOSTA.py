...
class SquareMoveOdom(SquareMove):
    ...
    def turn_of(self, a, ang_speed=0.4):
        ...
        a_init = abs(self.get_z_rotation(self.odom_pose.orientation))
        print a_init

        # Set the angular velocity forward until angle is reached
        while (abs(abs(self.get_z_rotation(self.odom_pose.orientation)) - a_init)) < a and not ros.is_shutdown():
	...
	time.sleep(1) #Time Delay
        ...
        self.turn_of(abs((math.pi/2))-0.4) #4th CCW Rotation
	time.sleep(1) #Time Delay
	self.turn_of(0) #Confirms End of Final Rotation
