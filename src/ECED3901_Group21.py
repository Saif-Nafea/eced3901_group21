...
class SquareMoveOdom(SquareMove):
	...
	self.turn_of(math.pi/2) #1st CCW Rotation
        ...
        self.turn_of(abs(abs((math.pi/2))-0.4)) #2nd CCW Rotation
	time.sleep(1) #Time Delay
	self.turn_of(0) #Elimates possibility of extra rotation
	
        self.move_of(0.5) #2nd Left Move
        self.turn_of(abs(abs((math.pi/2))-0.4)) #3rd CCW Rotation
	time.sleep(1) #Time Delay
        self.turn_of(0) #Elimates possibility of extra rotation
	
        self.move_of(0.5) #3rd Left Move
	self.move_of(0) #Elimates possibility of extra forward movement

	self.turn_of(0) #Elimates possibility of extra rotation
        self.turn_of(math.pi/2) #4th CCW Rotation
        self.turn_of(0) #Elimates possibility of extra rotation
